...
export const api = {
    getPartnerScheduleList: async (payload) => {
        const { partnerId, day } = payload;
        return await api_manager.get(`/schedule2/${partnerId}/${day}`);
    },
    postPartnerSchedule: async (payload) => {
        return await api_manager.post("/schedule2/partner", payload);
    },
    deletePartnerSchedule: async (payload) => {
        const { partnerId, day } = payload;
        return await api_manager.remove(
            `schedule2/partner?partnerId=${partnerId}&day=${day}`,
            payload
        );
    },
};
...
function* postPartnerScheduleFunc(action) {
    const { partnerId, day, schedules } = action.payload;
    try {
